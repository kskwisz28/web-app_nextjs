  ...
  return (
    <Container>
      ...
      <Grid gap={5} columns={[1, null, 3]}>
        <RangeSlider triggerTextAnimation={textAnimation} value={1000} min={0} max={100000} step={10000}
                     suffix="besökare" title="Besökare"/>
        <RangeSlider triggerTextAnimation={textAnimation} value={10000000} min={0} max={100000000} step={10000000}
                     suffix="kr" title="Din omsättning"/>
        <RangeSlider triggerTextAnimation={textAnimation} value={1000} min={0} max={10000} step={1000} suffix="st"
      ...
      <Grid mt={4} columns={[1, null, 2]} gap={"0px 1rem"} css={{alignItems: 'center'}}>
        ...
        <Listbox value={selectedPricing} onChange={onPlanChange}>
          <Listbox.Button sx={{
            '@media screen and (max-width: 540px)': {mx: 'auto'},
            maxWidth: '165px',
            boxShadow: '0 8px 14px rgb(50 50 93 / 10%), 0 4px 6px rgb(0 0 0 / 8%)',
            borderRadius: '0',
            background: '#6154F9',
            color: '#fff',
            border: 'none',
            cursor: 'pointer',
            padding: '1rem',
            ':hover': {background: '#F44599'}
          }}>{selectedPricing.name} <small>{selectedPricing.type}</small></Listbox.Button>
          <div></div>
          <Listbox.Options
            sx={{mx: 'auto', width: '100%', padding: '1rem', background: '#362f4a', color: '#fff'}}>
              ...
              {pricing.map((plan) => (
                ...
                >
                  <li sx={{
                    ...
                    py: '1rem',
                    cursor: 'pointer',
                    ':hover': {opacity: '0.8', background: 'rgba(255,255,255,.1)'}
                  }}>
                    {plan.unavailable ? <div css={{position: 'relative'}}>
                        <div sx={{opacity: '0.4', width: '100%'}}><span
                          css={{background: plan.color, padding: '0.2rem 0.8rem'}}>{plan.name}</span><small
                          css={{display: 'block'}}>{plan.type}</small></div>
                        <div sx={{position: 'absolute', top: 0, left: 0, textAlign: 'center', width: '100%'}}>Ej
                          tillgängligt
                        </div>
                      </div> :
                      <div><span css={{background: plan.color, padding: '0.2rem 0.8rem'}}>{plan.name}</span><small
                        css={{display: 'block'}}>{plan.type}</small></div>}
                  </li>
                </Listbox.Option>
              ))}
      ...
      <Grid mt={4} mb={5} columns={[1, null, 2]} gap={"0px 1rem"}
        ...
        <motion.div css={{'@media screen and (max-width: 540px)': {textAlign: 'center'}}}
                    transition={{ease: "easeInOut", duration: 1}} animate={controls}><Text css={{
          borderBottom: '2px solid #1ab876',
          color: '#1ab876',
          fontWeight: '600'
        }}>Alltid {selectedPricing.price}:- <small>per månad</small></Text></motion.div>
      </Grid>
    </Container>
  );
}
